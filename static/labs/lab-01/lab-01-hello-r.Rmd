...
The main goal of this lab is to introduce you to R and RStudio, which we will be using throughout the course to analyze data as well as to learn the statistical concepts discussed in the course. 
...
## 2. Accept assignment and join course organization:
...
```{r github-classroom, fig.margin = TRUE, echo = FALSE, fig.width=3}
...
- In the next screen, click on **Accept this assignment**. This will create the  GitHub repository (which we'll refer to as "repo" going forward) for the assignment. This repo contains a template you can build on to complete your assignment. Click on the repository link created for you to get started.

## 3. Clone the repo

The starter code you will build upon for this assignment now lives in the GitHub 
repo you just created. The next step is to bring this code into RStudio so that 
you can edit it, run it, view your results, and interpret them. This action is 
called *cloning*.
...
- Go to the course webpage, and launch RStudio: 
...
In this lab we will work with three packages: `datasauRus` which contains the dataset, `tidyverse` which is a collection of packages for doing data analysis in a "tidy" way, and `usethis` for setting our Git credentials.
...
```{marginfigure}
Not sure how to use emojis on your computer? Maybe a teammate can help? Or you can ask your tutor as well!
```

...
11. What is the definition of adjusted  $R^2$? Explain how it is different than 
    $R^2$ using values from this model.
    
12. How do the adjusted $R^2$ values of `m_bty_gen` and `m_bty` compare? What does this tell us 
...
13. Compare the slopes of `bty_avg` under the two models (`m_bty` and `m_bty_gen`). Has the 
    addition of `gender` to the model changed the parameter estimate (slope) for `bty_avg`?
    
14. Create a new model called `m_bty_gen_rank` predicting average professor evaluation 
...
15. How do the adjusted $R^2$ values of `m_bty_gen_rank` and `m_bty_gen` compare? 

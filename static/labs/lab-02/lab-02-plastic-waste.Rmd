...
Plastic pollution is a major and growing problem, negatively affecting oceans and wildlife health. [Our World in Data](https://ourworldindata.org/plastic-pollution) has a lot of great data at the various levels including globally, per country, and over time. For this lab we focus on data from 2010.
...
Go to the course [GitHub organization](https://github.com/ids-s1-19) and locate your HW 1 repo, which should be named `lab-02-plastic-waste-YOUR_GITHUB_USERNAME`. Grab the URL of the repo, and clone it in RStudio. Refer to [Lab 01](/labs/lab-01/lab-01-hello-r) if you would like to see step-by-step instructions for cloning a repo into an RStudio project.
...
```{r soln1, echo=FALSE, include=FALSE}
...
```{r soln2, echo=FALSE, include=FALSE}
...
```{r soln3, echo=FALSE, include=FALSE}
...
```{r soln4, echo=FALSE, include=FALSE}
ggplot(data = plastic_waste, 
       mapping = aes(x = plastic_waste_per_cap, 
...
```{r soln5, echo=FALSE, include=FALSE}
ggplot(data = plastic_waste, 

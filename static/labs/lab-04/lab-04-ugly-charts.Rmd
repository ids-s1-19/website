...
Go to the course [GitHub organization](https://github.com/ids-s1-19) and locate your Lab 04 repo, which should be named `lab-03-ugly-charts-YOUR_TEAMNAME`. Grab the URL of the repo, and clone it in RStudio. Refer to [Lab 01](/labs/lab-01/lab-01-hello-r) if you would like to see step-by-step instructions for cloning a repo into an RStudio project.

First, open the R Markdown document `lab-03-ugly-charts.Rmd` and Knit it. Make sure it compiles without errors. The output will be in the file markdown `.md` file with the same name.
...
The current version of a tidyverse package we will use today, **tidyr**, installed 
in Noteable is slightly out of date (v0.8.3 vs v1.0.0). To fix this run the following
in the Console,
...
Then, run the following code in the Console to load this package.
